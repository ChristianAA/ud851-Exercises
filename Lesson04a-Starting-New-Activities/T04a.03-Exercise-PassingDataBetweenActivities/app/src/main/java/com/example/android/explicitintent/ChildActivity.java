...
public class ChildActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Intent intentThatStartedChildActivityIntent = getIntent();
        // COMPLETED (4) Create an if statement to check if this Intent has the extra we passed from MainActivity
        if (intentThatStartedChildActivityIntent.hasExtra(Intent.EXTRA_TEXT)) {
            // COMPLETED (5) If the Intent contains the correct extra, retrieve the text
            String enteredText = intentThatStartedChildActivityIntent.getStringExtra(Intent.EXTRA_TEXT);
